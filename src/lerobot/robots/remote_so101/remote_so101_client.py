...
from .config_remote_so101 import RemoteSO101ClientConfig


class RemoteSO101Client(Robot):
    config_class = RemoteSO101ClientConfig
    name = "remote_so101_client"

    def __init__(self, config: RemoteSO101ClientConfig):
    ...
    def _remote_state_from_obs(
        ...
        flat_state = {key: observation.get(key, 0.0)
                      for key in self._state_order}

        state_vec = np.array([flat_state[key]
                             for key in self._state_order], dtype=np.float32)

        obs_dict: dict[str, Any] = {**flat_state, OBS_STATE: state_vec}

        # Decode images
        print(self._cameras_ft.keys())
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        """Command remote_so101 to move to a target joint configuration. Translates to motor space + sends over ZMQ
        ...
        self.zmq_cmd_socket.send_string(
            json.dumps(action))  # action is in motor space

        # TODO(Steven): Remove the np conversion when it is possible to record a non-numpy array value
        actions = np.array([action.get(k, 0.0)
                           for k in self._state_order], dtype=np.float32)

        action_sent = {key: actions[i]
    ...
    def disconnect(self):
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "RemoteSO101 is not connected. You need to run `robot.connect()` before disconnecting."
            )
